...
class ImppServer extends EventEmitter {
  ...
  _onConnection(conn) {
    ...
    this.protocol.handshake(conn, (success, id) => {
      ...
      // upgrade connection
      let imppClient = new ImppClient(conn, id, this.protocol);

      imppClient.on('message', (message) => {
        this.messageProcessor.process(message, imppClient, this);
      });

      imppClient.once('close', () => {
